    ...
    <ProtectedRoute>
      ...
      <div className="p-6 min-h-screen bg-gradient-to-br from-[#FFF8F0] to-[#FAEDCD] text-[#344E41]">
        ...
        <motion.form
          ...
          className="bg-white/80 border border-[#DAD7CD] backdrop-blur-md p-6 rounded-2xl shadow grid grid-cols-1 gap-4 mb-8"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        >
          {/* Baris Input */}
          ...
          {/* Input File */}
          <input type="file" accept="image/*" onChange={(e) => setForm({ ...form, gambar: e.target.files[0] })} className="w-full p-2 border rounded bg-[#f6f6f6] focus:outline-none focus:ring-2 focus:ring-[#A3B18A]" />

          {/* Tombol Submit */}
          <button type="submit" className="w-full bg-[#A3B18A] text-white py-2 rounded hover:bg-[#588157] transition">
